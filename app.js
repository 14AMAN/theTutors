...
app.get("/resetPasswordT/:token", function (req, res) {
  ...
  tutor.findOne(
    ...
    function (err, foundUser) {
      if (err) {
        console.error("Error finding user:", err);
        return res.render("/resetPasswordT/" + token, {
  ...
  if (req.isAuthenticated()) {
    ...
    if (req.user.role === "tutor") {
      ...
      res.render("managecourses", {
        msg: "",
        courses: req.user.purchasedCourse,
      });
    }
  } else {
    res.redirect("login");
...
app.post("/course/:param", async function (req, res) {
  if (req.isAuthenticated() && req.user.role === "tutor") {
    ...
    try {
      ...
      const updatedCourse = await courseDB.findOneAndUpdate(
        { _id: courseId },
        { courseName, fees, description, classs },
        { new: true }
      );
...
app.get("/post/:param", function (req, res) {
  async function post() {
    await story.find({ _id: req.params.param }).then((result) => {
...
async function handlePaymentSuccess(paymentId, courseId, sID, name, _class) {
  try {
      ...
      async function copyCourse(courseId) {
        try {
          ...
          await student.updateOne(
            { _id: sID },
            { $push: { purchasedCourse: courseInStudent } }
          );
    ...
    console.error(
      "Error while saving transaction and updating student:",
      error
    );
...
app.get("/payment-success", (req, res) => {
  if (req.isAuthenticated() && req.user.role == "student") {
    ...
    async function execute() {
      let _success = await handlePaymentSuccess(
        paymentId,
        courseId,
        req.user._id,
        req.user.name,
...
const start = async () => {
  await mongoose.connect(
    "mongodb+srv://aman7869211:OYOiYLN1AbP1LaEA@cluster0.zrmi4vs.mongodb.net/?retryWrites=true&w=majority"
  );
